...
<p>可以通过设置“凭据模式”：</p>
<ol>
<li><code v-pre>ananymous</code> 表示不包含凭据，就是不设置cookie，跨源的时候不在请求中附加值，保持匿名请求</li>
<li><code v-pre>use-credentials</code>表示在请求中包含凭据，cookie、tls证书、http认证（用户身份）</li>
<li>不设置这个属性，或者值是&quot;&quot;，相当于<code v-pre>ananymous</code></li>
</ol>
<h4 id="intergrity" tabindex="-1"><a class="header-anchor" href="#intergrity" aria-hidden="true">#</a> intergrity</h4>
<p>完整性。涉及子资源完整性，子资源指通过html页面引用的外部资源。</p>
<p>为了防止外部攻击篡改子资源。</p>
...
<li>Execution 执行脚本依赖等。</li>
</ol>
<p><code v-pre>defer</code>属性会异步加载脚本，但是执行脚本是同步的</p>
<p><code v-pre>async</code>属性是异步加载和异步执行，加载完成脚本之后就执行</p>
<p><code v-pre>type=module</code>默认是使用<code v-pre>defer</code>的，从入口文件开始，碰到依赖后，加载依赖，加载完依赖之后，再回到入口文件。  如果不依赖html文档文件的话，可以在加上<code v-pre>async</code>标签，加载完文件之后就可以执行execution。</p>
</div></template>
