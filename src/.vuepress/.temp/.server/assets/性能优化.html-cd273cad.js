...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs) {
  ...
  _push(`<div${ssrRenderAttrs(_attrs)}><h1 id="性能优化" tabindex="-1"><a class="header-anchor" href="#性能优化" aria-hidden="true">#</a> 性能优化</h1><h2 id="性能优化的目的" tabindex="-1"><a class="header-anchor" href="#性能优化的目的" aria-hidden="true">#</a> 性能优化的目的</h2><ol><li>首屏时间</li><li>首屏可交互时间</li><li>首次有意义内容渲染时间</li></ol><p>性能检测： <a href="https://developers.google.com/speed/pagespeed/insights" target="_blank" rel="noopener noreferrer">https://developers.google.com/speed/pagespeed/insights`);
  _push(ssrRenderComponent(_component_ExternalLinkIcon, null, null, _parent));
  _push(`</a></p><p>polyfill: <a href="https://polyfill.io/v3/url-builder" target="_blank" rel="noopener noreferrer">https://polyfill.io/v3/url-builder`);
  _push(ssrRenderComponent(_component_ExternalLinkIcon, null, null, _parent));
  _push(`</a></p><h2 id="优化方法" tabindex="-1"><a class="header-anchor" href="#优化方法" aria-hidden="true">#</a> 优化方法</h2><h3 id="只请求当前需要的资源" tabindex="-1"><a class="header-anchor" href="#只请求当前需要的资源" aria-hidden="true">#</a> 只请求当前需要的资源</h3><ul><li>懒加载</li><li>异步加载</li><li><a href="https://www.notion.so/polyfill-04d4d93743e94a8aa388c01a71e935da" target="_blank" rel="noopener noreferrer">polyfill`);
  _push(ssrRenderComponent(_component_ExternalLinkIcon, null, null, _parent));
  _push(`</a></li></ul><h3 id="缩减资源体积" tabindex="-1"><a class="header-anchor" href="#缩减资源体积" aria-hidden="true">#</a> 缩减资源体积</h3><ul><li>打包压缩，webpack4版本已经内置</li><li>gzip，一种压缩算法，一般默认开启</li><li>图片格式的优化，压缩 <a href="http://tinypng.com" target="_blank" rel="noopener noreferrer">tinypng.com`);
  _push(ssrRenderComponent(_component_ExternalLinkIcon, null, null, _parent));
  _push(`</a>,根据屏幕分辨率展示不同分辨率的图片，webp(可以使图片在无损的情况下进行压缩，需要判断浏览器是否支持webp）</li><li>尽量控制cookie的大小，每一个request header都要携带cookie，同域名请求都会带上所有的cookie，耗费多余的体积。</li></ul><h3 id="时序优化" tabindex="-1"><a class="header-anchor" href="#时序优化" aria-hidden="true">#</a> 时序优化</h3><p>通过一些代码能力影响资源的加载。</p><ul><li><code>promise.all</code> 并行执行promise，并发发请求</li><li><code>ssr</code> 把打包放到服务端，好处是可以做缓存，更新更新的地方，并且seo友好</li><li><code>prefetch, prerender, preload</code></li></ul><div class="language-jsx line-numbers-mode" data-ext="jsx"><pre class="language-jsx"><code><span class="token comment">// 碰到这行代码后会立即请求dns解析，也就是预解析</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="合理的利用缓存" tabindex="-1"><a class="header-anchor" href="#合理的利用缓存" aria-hidden="true">#</a> 合理的利用缓存</h3><p>cdn、cdn预热、cdn刷新。</p><p>cnd域名和业务域名通常是不一样的，为了避免cookie影响，增加请求消耗。</p><h2 id="如果一段js执行时间非常长-怎么去分析" tabindex="-1"><a class="header-anchor" href="#如果一段js执行时间非常长-怎么去分析" aria-hidden="true">#</a> 如果一段js执行时间非常长，怎么去分析</h2><div class="language-tsx line-numbers-mode" data-ext="tsx"><pre class="language-tsx"><code><span class="token comment">// 封装一个计时的装饰器函数</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="阿里云oss支持通过链接后面品参数来做图片的格式转换-尝试写一下-把任意图片格式转换为webp-需要注意什么" tabindex="-1"><a class="header-anchor" href="#阿里云oss支持通过链接后面品参数来做图片的格式转换-尝试写一下-把任意图片格式转换为webp-需要注意什么" aria-hidden="true">#</a> 阿里云oss支持通过链接后面品参数来做图片的格式转换，尝试写一下，把任意图片格式转换为webp，需要注意什么</h3><p>首先应该判断浏览器是否支持webp。canisuse.com可以看浏览器支持性。</p><p>封装一个判断方法：</p><div class="language-tsx line-numbers-mode" data-ext="tsx"><pre class="language-tsx"><code><span class="token keyword">function</span> <span class="token function">checkWebp</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>通过创建一个 <code>canvas</code> 元素，将它转为base64格式的image/webp，如果浏览器支持，它的开头将带有 <code>data:image/webp</code> 的字符串。（如代码注释）。如果不存在或者报错，则返回false。</p><p>接下来看一下如何经过判断去拼接阿里oss：</p><div class="language-tsx line-numbers-mode" data-ext="tsx"><pre class="language-tsx"><code><span class="token keyword">const</span> supportWebp <span class="token operator">=</span> <span class="token function">checkWebp</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>注意一个点，以上是默认图片都在oss上的</p><h3 id="如果有巨量的图片需要展示-除了懒加载的方式-还有没有其他方法限制一下同时加载的图片数量" tabindex="-1"><a class="header-anchor" href="#如果有巨量的图片需要展示-除了懒加载的方式-还有没有其他方法限制一下同时加载的图片数量" aria-hidden="true">#</a> 如果有巨量的图片需要展示，除了懒加载的方式，还有没有其他方法限制一下同时加载的图片数量</h3><p>代码题，实现promise的并发控制，例如要求同时进行3个图片的加载，始终加载3个</p><blockquote><p>这里不能用for循环+promise.all的原因是，后者是执行完3个再去执行3个，不符。</p></blockquote><div class="language-tsx line-numbers-mode" data-ext="tsx"><pre class="language-tsx"><code><span class="token keyword">function</span> <span class="token function">limitLoad</span><span class="token punctuation">(</span>urls<span class="token punctuation">,</span> handler<span class="token punctuation">,</span> limit<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("../pages/posts/性能优化/性能优化.html.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const _____html = /* @__PURE__ */ _export_sfc(_sfc_main, [["ssrRender", _sfc_ssrRender], ["__file", "性能优化.html.vue"]]);
export {
